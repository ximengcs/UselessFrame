...
{
    public class NLogLogger : UselessFrame.NewRuntime.ILogger
    {
        ...
        public NLogLogger()
        {
            LogManager.Setup().LoadConfiguration((builder) =>
            {
                ...
                builder.ForLogger().FilterLevel(LogLevel.Debug).WriteToFile($"logs/nlog-debug-{time}.log", layout, null, null, true, true, 107374182, 1);
                builder.ForLogger().FilterLevel(LogLevel.Warn).WriteToFile($"logs/nlog-warning-{time}.log", layout, null, null, true, true, 107374182, 1);
                builder.ForLogger().FilterLevel(LogLevel.Error).WriteToFile($"logs/nlog-error-{time}.log", layout, null, null, true, true, 107374182, 1);
                builder.ForLogger().FilterLevel(LogLevel.Fatal).WriteToFile($"logs/nlog-fatal-{time}.log", layout, null, null, true, true, 107374182, 1);
            });
